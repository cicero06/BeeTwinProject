...
class LoR            // Klasik anomali tespiti (backward compatibility)
            if (lastReading) {
                const basicAnomalies = this.detectAnomalies(lastReading.data, mappedSensorData);
    ...
    async ensureRouterConfiguration(hive, routerId, sensorId) {
        try {
            ...
            if (!existingRouter) {
                ...
                console.log('🔄 Router lastSeen updated:', routerId);
            }

        } catch (error) {
            console.error('❌ Error updating router configuration:', error.message);
        }
    }

    // Sensör oluşturma/bulma fonksiyonu
    async ensureSensorExists(wirelessData, hive, routerId, sensorId) {
        try {
            // Önce mevcut sensörü ara
            ...
            if (!sensor) {
                console.log('📝 Creating new sensor for Router:', routerId, 'Sensor:', sensorId);
                ...
                sensor = await Sensor.create({
                    ...
                    routerId: routerId,
                    sensorId: sensorId,
                    name: `${hive.name} - Router ${routerId}`,
                ...
                console.log('✅ New sensor created:', sensor.deviceId, 'Router:', routerId, 'Types:', sensorTypes);
            } else {
                // Mevcut sensörü güncelle
                sensor.lastSeen = new Date();
                sensor.batteryLevel = wirelessData.batteryLevel || sensor.batteryLevel;
                sensor.status = 'active';
                await sensor.save();
                console.log('🔄 Existing sensor updated:', sensor.deviceId);
            }

            return sensor;
        } catch (error) {
            console.error('❌ Error ensuring sensor exists:', error.message);
            return null;
        }
    }
    ...
    async processWirelessData(wirelessData) {
        try {
            ...
            // 🔧 PERMANENT SOLUTION: Ensure router configuration is up-to-date
            await this.ensureRouterConfiguration(hive, routerId, sensorId);

            // Sensörü database'de bul veya oluştur
            let sensor = await this.ensureSensorExists(wirelessData, hive, routerId, sensorId);

            if (!sensor) {
                console.error('❌ Sensör oluşturulamadı:', wirelessData.deviceId);
                return null;
            }
            ...
            // Field mapping - Koordinatör formatından database formatına çevir
            const mappedSensorData = this.mapCoordinatorFields(wirelessData.sensorData);
            console.log('🔄 Field mapping applied:', wirelessData.sensorData, '→', mappedSensorData);
    ...
    // Koordinatör field mapping - WT, PR, WH formatından backend formatına çevir
    mapCoordinatorFields(sensorData) {
        const fieldMapping = {
            // Çevresel sensörler
            "WT": "temperature",
            "WH": "humidity", 
