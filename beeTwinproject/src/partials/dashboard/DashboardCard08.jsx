...
function DashboardCard08() {
  ...
  // Saatlik veri aktivitesi analizi - Kullanıcıya özel filtre
  useEffect(() => {
    if (realTimeSensorData && realTimeSensorData.length > 0 && user) {
      // Kullanıcının kovanlarına ait verileri filtrele
      const userHiveIds = hives?.map(hive => hive.id) || [];

      // Kullanıcının verilerini filtrele
      const userSensorData = realTimeSensorData.filter(data =>
        !data.hiveId || userHiveIds.includes(data.hiveId) || userHiveIds.includes(data.hive_id)
      );
      ...
      const todayData = userSensorData.filter(data => {
