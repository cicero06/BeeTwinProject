...
class WebSocketService {
    ...
    connect(url = 'ws://localhost:3001') {
        ...
        // Development mode - skip actual WebSocket connection
        if (process.env.NODE_ENV === 'development') {
            console.log(`🚧 Development mode - Skipping WebSocket connection (attempt #${currentAttempt})`);
            console.log('📡 Real Data Mode: No WebSocket server in development');
            this.isConnecting = false;
            this.isConnected = false;
            // Notify components that we're in "no real-time data" mode
            setTimeout(() => {
                this.notifyListeners('connectionStatus', {
                    connected: false,
                    reason: 'Development mode - No WebSocket server'
                });
            }, 100);
